...
class Paging extends CI_Controller {
    ...
    public function djabes()
    {
        $this->load->model('DjabesUsers');

        $params = array();
        $limit_per_page = 10;
        $start_index = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
        $total_records = $this->DjabesUsers->get_total();

        if ($total_records > 0)
        {
            $params["results"] = $this->DjabesUsers->get_current_page_records($limit_per_page, $start_index);

            $config['base_url'] = base_url() . 'paging/djabes';
            $config['total_rows'] = $total_records;
            $config['per_page'] = $limit_per_page;
            $config["uri_segment"] = 3;
        ...
        $this->load->view('djabes_users', $params);
    }
